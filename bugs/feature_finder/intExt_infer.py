...
def analyze_infer(dimacs_, cdir_, jsonfile_):
    ...
    for datum in rawdata:
        #print(datum['investigation']['result'])

        if datum['investigation']['result']:
            # get list of configurations
            configs = datum['configurations']

            print(datum['node_key'] + ":" + str(datum['file']) + "," + str(datum['line']) + ": ", end='')

            # get filtered config list
            filtered = list()
            for c in configs:
                filtered.append(str(c) + ".config")

            filter_common(dimacs_, cdir_, filtered)
            i += 1
...
if __name__ == "__main__":
    ...
    analyze_infer(dimacs, sampledir, jsonfile)
